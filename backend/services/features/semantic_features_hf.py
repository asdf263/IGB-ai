"""
Semantic Feature Extraction Module with HuggingFace Transformers
Extracts semantic embedding features using HuggingFace models
"""
import numpy as np
...
class SemanticFeatureExtractorHF:
    """Extracts semantic embedding features using HuggingFace Transformers."""
    ...
    def _init_model(self):
        ...
        try:
            ...
            try:
                ...
                print("Neither sentence-transformers nor transformers installed")
                self.model = None
    ...
    def _compute_similarity_features(self, embeddings: np.ndarray) -> Dict[str, float]:
        ...
        hist, _ = np.histogram(upper_tri, bins=10, range=(-1, 1), density=True)
        hist = hist / (hist.sum() + 1e-10)
        entropy = -np.sum(hist * np.log(hist + 1e-10))
        
        return {
            'cosine_sim_mean': float(np.mean(upper_tri)),
            'cosine_sim_std': float(np.std(upper_tri)),
            'cosine_sim_min': float(np.min(upper_tri)),
            'cosine_sim_max': float(np.max(upper_tri)),
    ...
    def _estimate_cluster_count(self, embeddings: np.ndarray, threshold: float = 0.7) -> int:
        """Estimate number of topic clusters."""
        n = len(embeddings)
        if n < 2:
            return 1
    ...
    def _compute_content_density(self, embeddings: np.ndarray) -> float:
        ...
        if len(embeddings) < 2:
            return 0.0
        centroid = np.mean(embeddings, axis=0)
        avg_dist = np.mean(np.linalg.norm(embeddings - centroid, axis=1))
